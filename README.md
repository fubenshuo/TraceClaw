...
[![TraceClaw](https://img.shields.io/badge/TraceClaw-1.0.0-purple.svg?logo=cyberpunk)](https://github.com/ttguy0707/TraceClaw)
...
TraceClaw 是一个**企业级透明可控智能体**，基于 Harness Engineering 理念重新定义 AI 系统的可信边界：
...
> **Harness Engineering**：TraceClaw 不仅是一个 Agent 框架——它是构建在 LangGraph 之上的**可信执行层**。通过架构约束（沙盒 + 两段式调用）、全链路可观测性（5 类审计事件）、记忆管理（双水位）、反馈回路（心跳 → Agent → 飞书），为 AI Agent 提供了一套完整的"操作系统级"保障。

### 🔌 技能生态兼容

TraceClaw 支持**OpenClaw 技能**和**Claude Code 技能**，可直接使用两个生态系统的丰富技能资源，无需重新开发。
...
| **🏗️ Harness 约束层** | 沙盒隔离 + Shell 防线 + 两段式调用 + 心跳反馈 | 纵深防御，不靠单一机制扛所有风险 |
| **🔎 Trace 全链路审计** | agent_node 内置 4 层审计 + thread_id 可追溯 + 实时监控终端 | 嵌入决策循环，AI 每一步都有据可查 |
...
> 💡 **工作区配置**：工作区路径已在代码中初始化，默认为项目根目录的 `workspace` 文件夹，无需在 `.env` 中配置。仅当需要自定义工作区位置时，才设置 `TRACECLAW_WORKSPACE` 环境变量。

> 💡 提示：配置完成后，可运行 `traceclaw run` 聊天测试连接是否正常。
...
- **Harness 约束层** (橙色)：沙盒路径拦截 + Shell 命令过滤 + 两段式安全调用
- **Trace 透明监控层** (绿色)：LLM 输入审计 + 工具决策 + 工具参数 + 调用结果全链路记录
- **输出层** (底部)：聊天终端 + 监控终端
...
| 模块 | 文件 | 功能 |
|------|------|------|
| **Agent 循环** | `traceclaw/core/agent.py` | LangGraph StateGraph，决策大脑 |
| **技能加载** | `traceclaw/core/skill_loader.py` | 动态加载 SKILL.md，两段式调用 |
| **上下文管理** | `traceclaw/core/context.py` | 消息修剪，双水位记忆 |
| **内置工具** | `traceclaw/core/tools/builtins.py` | 时间/计算/任务调度等 |
